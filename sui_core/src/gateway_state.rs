...
pub struct GatewayState<A> {
    ...
    unsigned_tx: BTreeMap<TransactionDigest, TransactionData>,
}
    ...
    A: AuthorityAPI + Send + Sync + Clone + 'static,
{
    async fn execute_transaction(
        ...
        let (certificate, effects) = self
            .execute_transaction(Transaction::new(tx, signature))
            .await?;
        self.unsigned_tx.remove(&digest);

        match certificate.transaction.data.kind.clone() {
            TransactionKind::Call(_) | TransactionKind::Transfer(_) => {
                Ok(TransactionResponse::EffectResponse(certificate, effects))
            }
            TransactionKind::Publish(_) => self.to_publish_response(certificate, effects).await,
            TransactionKind::SplitCoin(call) => {
                self.to_split_coin_response(call, certificate, effects)
                    .await
            }
            TransactionKind::MergeCoin(call) => {
                self.to_merge_coin_response(call, certificate, effects)
                    .await
            }
        }
    }
    ...
    async fn transfer_coin(
        ...
        self.unsigned_tx.insert(data.digest(), data.clone());
    ...
    async fn split_coin(
        ...
        let data = TransactionData::new_split_coin_call(
            signer,
    ...
    async fn merge_coins(
        ...
        let data = TransactionData::new_merge_coin_call(
            signer,
            self.get_framework_object_ref().await?,
            coin::COIN_MODULE_NAME.to_owned(),
